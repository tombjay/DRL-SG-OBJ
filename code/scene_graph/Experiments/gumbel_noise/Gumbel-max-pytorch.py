"""
This file consists the code for the experiment Gumbel-max trick using Pytorch's 'gumbel_softmax' function. 
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, unprocessed_obs, global_step = 0, log_logits = False, action = None):
        ...
        # Compute log probabilities and entropy using the sampled actions
        log_probs = torch.log(probs)
        entropy = -torch.sum(probs * log_probs, dim=-1)
        
        if log_logits:
            writer.add_histogram('histograms/logits_histogram', logits, global_step, bins = 'auto')
        return action, log_probs.gather(1, action.unsqueeze(1)).squeeze(1), entropy, critic_value
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'Gumbel_noise_FB_mesh_topology_clipped_vloss_10M')
    run_name = f"{args.env_id}__Gumbel_noise_FB_mesh_topology_clipped_vloss_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb

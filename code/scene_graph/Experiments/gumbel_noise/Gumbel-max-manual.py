"""
This file consists the manual implementation of Gumbel-max trick
...
class NodeFeatureEmbedding(nn.Module):
    def __init__(self, bbox_in_dim = 4, bbox_out_dim = 4, color_in_dim = 3, color_out_dim = 6):
        super(NodeFeatureEmbedding, self).__init__()
        self.bbox_layer = nn.Linear(bbox_in_dim, bbox_out_dim)
        self.color_layer = nn.Linear(color_in_dim, color_out_dim)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, unprocessed_obs, global_step = 0, log_logits = False, action = None):
        ...
        gumbel_logits = logits - torch.log(-torch.log(noise + 1e-20))
        if action is None:
            action = torch.argmax(gumbel_logits, dim=-1)

        log_probs = torch.log_softmax(logits, dim=-1)
        selected_log_probs = torch.gather(log_probs, 1, action.unsqueeze(1)).squeeze(1)
        ...
        return action, selected_log_probs, entropy, critic_value
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'Gumbel_noise_FB_mesh_topology_10M')
    run_name = f"{args.env_id}__Gumbel_noise_FB_mesh_topology_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb

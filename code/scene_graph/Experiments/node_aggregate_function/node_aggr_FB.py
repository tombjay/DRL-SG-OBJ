"""
This file consists the code for node aggregation experiment for Frostbite environment.
Here we change the GCNConv model's default message passing aggregation function from 'sum' to 'mean'
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 8, hidden_dim = 128):
        ...
        self.GCNConv = Seq('x, edge_index', [
            (GCNConv(in_channels = input_node_dim, out_channels = hidden_dim, aggr = 'mean'), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
            (GCNConv(in_channels = hidden_dim, out_channels = hidden_dim, aggr = 'mean'), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
        ])
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'node_aggr_FB_10M')
    run_name = f"{args.env_id}__node_aggr_FB_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb

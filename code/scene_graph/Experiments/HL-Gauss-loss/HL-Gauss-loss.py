"""
This file consists the code for HL-Gauss method based value loss scaling.
In order to test this experiment for other environments, change the 'env_id' and the resepctive scene graph generation methods. 
...
def evaluate_model(eval_env, agent, global_step):
    ...
    for r in range(10):  # Perform 10 evaluation episodes
        ...
        while True:  # Continue until all environments are done
            ...
            with torch.no_grad():
                action, _, _, _ = agent.get_action_and_value(obs, global_step = global_step, log_logits = False)
...
class NodeFeatureEmbedding(nn.Module):
    ...
    def bbox_embedding(self, bounding_boxes):
...
global_color_embedding = [
    torch.tensor([ 31.4168, -19.4094,  81.4173]),
    torch.tensor([ 6.5051, -76.1086, 131.8404]),
    torch.tensor([ -25.2518, -70.6512,  67.6456]),
    torch.tensor([ -29.2403, -103.2694,   88.2510]),
    torch.tensor([ 4.1393, -50.3823,  87.3720]),
    torch.tensor([ 20.3818,  4.4321, 89.0003]),
    torch.tensor([ 32.7085, -40.8796, 108.5506]),
    torch.tensor([ 49.1313,   0.4642, 123.8499]),
    torch.tensor([  3.1207, -39.3056,  68.2260]),
    torch.tensor([  -25.2518, -70.6512,  67.6456]),
    torch.tensor([ -29.2403, -103.2694,   88.2510])
]
...
def create_graph_input(frames):
    ...
    for boxes, labels in zip(batch_boxes, batch_labels):
        ...
        for i, label in enumerate(labels):
            ...
            node_feature = bounding_box_embedding.bbox_embedding(bbox)
...
# Define the HL-Gauss loss
class HLGaussLoss(nn.Module):
    def __init__(self, min_value, max_value, num_bins, sigma):
    ...
    def forward(self, logits_HL, target):
        logits_HL = logits_HL.view(-1)
        cross_entropy_loss = F.cross_entropy(self.transform_to_probs(logits_HL), self.transform_to_probs(target))
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, unprocessed_obs, global_step = 0, log_logits = False, action = None):
        ...
        if log_logits:
            writer.add_histogram('histograms/logits_histogram', logits, global_step, bins = 'auto')
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'HL_loss_FB_10M')
    run_name = f"{args.env_id}__HL_loss_FB_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.evaluate and iteration % args.evaluation_interval == 0:
            performance = evaluate_model(envs, agent, global_step)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                if iteration % 100 == 0:
                    action, logprob, _, value = agent.get_action_and_value(next_obs, global_step = global_step, log_logits = True)
                else:
                    action, logprob, _, value = agent.get_action_and_value(next_obs, global_step = global_step, log_logits = False)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], global_step = global_step, action = b_actions.long()[mb_inds])
                ...
                # HL-Gauss loss
                hl_gauss_loss = HLGaussLoss(min_value= -200, max_value= 200, num_bins=21, sigma=0.75)
                newvalue = newvalue.view(-1)
                value_target = b_returns[mb_inds]
                v_loss = hl_gauss_loss(newvalue, value_target)

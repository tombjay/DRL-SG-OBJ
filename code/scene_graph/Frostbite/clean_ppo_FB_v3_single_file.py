...
and with unprocessed node embeddings of size 17.
...
def create_graph_input(frames):
    ...
    for boxes, labels in zip(batch_boxes, batch_labels):
        ...
        for i, label in enumerate(labels):
            ...
            label_encoding = [0] * len(object_id_mapping)  # Assuming label_values is a list of all possible labels
            label_encoding[label] = 1
            label_tensor = torch.tensor(label_encoding)
            ...
            node_feature = torch.cat((node_feature, label_tensor, friendly_indicator))
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 17, hidden_dim = 128, gcn_layers = 2):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GCN_Clean_FB_single_file_v3_10M')
    run_name = f"{args.env_id}__GCN__FB__single_file_v3{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb

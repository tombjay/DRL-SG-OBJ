...
and with unprocessed node embeddings of size 6.
The input graph is processed from a different file located at '/scratch/users/sundararaj/msc2023_jayakumar/fasterRCNN/sgg/graph_batch_process_FB.py'
...
from graph_batch_process_FB import graph_batch
...
def evaluate_model(eval_env, agent):
    episodic_returns = []
    for _ in range(10):  # Perform 10 evaluation episodes
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 6, hidden_dim = 128, gcn_layers = 2):
        super().__init__()
    ...
    def get_value(self, unprocessed_obs):
        ...
        batched_graph_obs = graph_batch(unprocessed_obs)
    ...
    def get_action_and_value(self, unprocessed_obs, action = None):
        ...
        critic_values = []
        b_graph_obs = []
        t_graph_obs = graph_batch(unprocessed_obs)
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GCN_Clean_FB_batch_10M')
    run_name = f"{args.env_id}__GNN__TEST2__GCN_FB_batch{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    if args.evaluate:
        eval_env = make_env(args.env_id, 0, args.capture_video, run_name)()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.evaluate and iteration % args.evaluation_interval == 0:
            performance = evaluate_model(eval_env, agent)
            wandb.log({"iter": iteration, "episodic_returns": performance})

...
and with processed node embeddings of size 8.
--> This is the implementation of star-topology based input scene graph.
...
def adjust_batch_size(node_indexes):
    length = len(node_indexes)
    if length <= 8:
        return [node_indexes[i] if i < length else -1 for i in range(8)]
    else:
        return [node_indexes[i] if i < length else -1 for i in range(256)]
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 8, hidden_dim = 128):
        ...
        self.GCN_layer = Seq('x, edge_index', [
    ...
    def forward(self, graph_data, num_graphs, ptr):
        ...
        gcn_output = self.GCN_layer(x = graph_data.x, edge_index = graph_data.edge_index)
        node_index = graph_data.edge_index[0].tolist()
        node_index_to_filter = graph_data.edge_index[1].tolist()
        final_node_index = sorted(set(node_index))
        final_node_index_to_filter = sorted(set(node_index_to_filter))
        final_node_index_filtered = [x for x in final_node_index if x not in final_node_index_to_filter]
        final_node_index_filtered = adjust_batch_size(final_node_index_filtered)
        batch_size = len(final_node_index_filtered)
        selected_features = torch.zeros(batch_size, gcn_output.size(1)).to(device)
        for i, node_index in enumerate(final_node_index_filtered):
            if node_index == -1:
                selected_features[i] = torch.zeros(1, gcn_output.size(1)).to(device)
            else:
                selected_features[i] = gcn_output[node_index]

        hidden = self.fc_network(selected_features)
        return hidden
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GCN_FB_Star_player_v6_10M')
    run_name = f"{args.env_id}_GCN_FB_Star_player_v6_10M_{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], action = b_actions.long()[mb_inds])

...
The input graph is processed from a different files:
Graph processing: '/scratch/users/sundararaj/msc2023_jayakumar/fasterRCNN/sgg/graph_processor.py'
Scene graph generation: '/scratch/users/sundararaj/msc2023_jayakumar/fasterRCNN/sgg/scene_graph_generation.py'
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 5, hidden_dim = 64, gcn_layers = 2):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GCN_clean_10M')
    run_name = f"{args.env_id}__GNN__TEST4__GCN_savelogic{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        try:
            ...
            print("ValueError occurred. Entering debug mode...")
            pdb.set_trace()

...
def evaluate_model(eval_env, agent, global_step):
    ...
    for r in range(10):  # Perform 10 evaluation episodes
        ...
        while True:  # Continue until all environments are done
            ...
            with torch.no_grad():
                action, _, _, _ = agent.get_action_and_value(obs, global_step = global_step)
...
def adjust_batch_size(node_indexes):
    ...
    if length <= 8:
        return [node_indexes[i] if i < length else 0 for i in range(8)]
    else:
        return [node_indexes[i] if i < length else 0 for i in range(256)]
...
class Agent(nn.Module):
    ...
    def forward(self, graph_data, num_graphs, ptr):
        ...
        final_node_index = adjust_batch_size(final_node_index)
        batch_size = len(final_node_index)
        selected_features = torch.zeros(batch_size, gcn_output.size(1)).to(device)
        for i, node_index in enumerate(final_node_index):
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GCN_SPI_v9_player_10M')
    run_name = f"{args.env_id}__GCN_SPI_v9_player_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.evaluate and iteration % args.evaluation_interval == 0:
            performance = evaluate_model(envs, agent, global_step = global_step)

...
and with processed node embeddings of size 5 and without edge embeddings.
"""
import os
import random
import time
...
def generate_bounding_boxes(frames):
    # Initialize lists to store boxes and labels for each frame
    batch_boxes = []
    batch_labels = []
...
def create_graph_input(frames):
    ...
    for boxes, labels in zip(batch_boxes, batch_labels):
        ...
        for i, label in enumerate(labels):
            ...
            # Concatenate bounding box coordinates and friendly indicator
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GATv2_SPI_Single_file_10M')
    run_name = f"{args.env_id}__GATv2_SPI_Single_file_10M{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb

...
and with processed node embeddings of size 5 and edge embeddings of size 5.
"""
import os
import random
import time
...
from utils_sgg import preprocess_image, generate_bounding_boxes
from scene_graph_generation import SceneGraphGeneration
...
class Args:
    ...
    """Whether to load model (check out 'checkpoints' folder)"""
...
def process_obs(observation, width = 160, height = 210):
    
    boxes, labels = generate_bounding_boxes(observation, width, height)
    scene_graph_generator = SceneGraphGeneration(boxes, labels)
    scene_graph = scene_graph_generator.generate_scene_graph()
    processor = GraphProcessor(scene_graph)
    graph_observation = processor.create_pytorch_geometric_data()
...
class Agent(nn.Module):
    def __init__(self, envs, input_node_dim = 5, hidden_dim = 128, gcn_layers = 2):
        ...
        self.GATv2Conv = Seq('x, edge_index, edge_attr', [
            (GATv2Conv(in_channels = input_node_dim, out_channels = hidden_dim, edge_dim = 5), 'x, edge_index,edge_attr -> x'),
            nn.ReLU(inplace=True),
            (GATv2Conv(in_channels = hidden_dim, out_channels = hidden_dim, edge_dim = 5), 'x, edge_index, edge_attr -> x'),
            nn.ReLU(inplace=True),
        ])
    ...
    def forward(self, graph_data, num_graphs, ptr):
        gcn_output = self.GATv2Conv(x = graph_data.x, edge_index = graph_data.edge_index, edge_attr = graph_data.edge_attr)
    ...
    def get_value(self, unprocessed_obs):
        ...
        for i in range(unprocessed_obs.shape[0]):
            obs = unprocessed_obs[i,:,:,:]
            graph_obs = process_obs(obs.cpu().numpy())
            b_graph_obs.append(graph_obs)
        
        batched_graph_obs = Batch.from_data_list(b_graph_obs)
    ...
    def get_action_and_value(self, unprocessed_obs, action = None):
        ...
        t_graph_obs = Batch.from_data_list(b_graph_obs)
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(args.model_path, 'GATv2_with_edgeattr_10M')
    run_name = f"{args.env_id}__GNN__TEST1__GMXPool{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        try:
            ...
            print("IndexError occurred. Entering debug mode...")
            pdb.set_trace()
